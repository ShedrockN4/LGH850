...
BUILD_NOW()
{
	...
	time make ARCH=arm64 CROSS_COMPILE=../aarch64-linux-android-4.9/bin/aarch64-linux-android- modules -j ${NR_CPUS} || exit 1
	...
	cp -a ../ramdisk/* ../Ramdisk-LGG5-MM-tmp/
	...
	#	rm -rf boot
		...
		zip -r Xp-Kernel-MM-"$(date +"%d.%m")"-"${GETVER}".zip * >/dev/null
		stat boot.img
		rm -f ./*.img
		cd ..
	else
